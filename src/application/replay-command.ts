...
import {
  ...
  EXACT_V1_VERSION,
} from "../judging/comparator/index.js";
...
export interface ReplayDependencies {
  ...
  readonly artifacts: ArtifactLookup;
...
export async function executeReplayCommand(
  command: ReplayInvocation,
  d: ReplayDependencies,
): Promise<ReplayOutcome> {
  try {
    const metadata = await d.artifacts.findById(command.artifactId);
    if (metadata === null || metadata.sha256 !== command.artifactId)
      return failure(
        "invalid_input",
        "artifact_not_found",
    ...
    if (!isDescendant(d.invocationDirectory, artifactPath))
      return failure(
        "invalid_input",
        "artifact_path",
        "artifact path escapes the invocation root",
      );
    const artifact = await loadFuzzArtifact(artifactPath, command.artifactId);
    if (
      !isSupportedCodecVersion(artifact.inputCodecVersion) ||
      !isSupportedCodecVersion(artifact.outputCodecVersion) ||
      artifact.comparatorVersion !== EXACT_V1_VERSION
    )
      return failure(
        "invalid_input",
        "unsupported_recorded_version",
        "artifact records unsupported codec or comparator versions",
      );
    const prepared = await prepare(artifact, command, d);
    const runId = toRunId(identifier(d));
    const caseId = toCaseId(identifier(d));
    const recordedRequest = Buffer.from(
      artifact.minimizedInputBase64Url,
      "base64url",
    );
    if (recordedRequest.length > artifact.limits.inputBytes)
      return failure(
        "invalid_input",
        "artifact_input_limit",
        "recorded input exceeds its recorded limit",
      );
    const decodedRequest = decodeRequestLine(recordedRequest);
    if (
      !decodedRequest.ok ||
      decodedRequest.envelope.codecVersion !== artifact.inputCodecVersion
    )
      return failure(
        "invalid_input",
        "artifact_protocol",
        "artifact input request is invalid",
      );
    const inputBytes = encodeRequestLine({
      protocolVersion: 1,
      kind: "request",
      runId,
      caseId,
      codecVersion: artifact.inputCodecVersion,
      messageLimitBytes: artifact.limits.inputBytes,
      input: decodedRequest.envelope.input,
    });
    const base = {
      runId,
      caseId,
      problemFingerprint: prepared.context.problem.slug,
      inputCodecVersion: artifact.inputCodecVersion,
      outputCodecVersion: artifact.outputCodecVersion,
      limits: artifact.limits,
      generation: initialGeneration(),
    };
    const naive = await executeEncodedCase(
      prepared.context,
      {
        ...base,
        inputBytes,
    ...
    const reproduced =
      predicate !== null && samePredicate(predicate, artifact.predicate);
    await persistReplay(
      prepared.context,
      artifact,
      command.artifactId,
    ...
    return {
      status,
      result: { runId, artifactId: command.artifactId, reproduced, predicate },
      diagnostics: [],
    };
  } catch (error) {
    return failure(
...
async function prepare(
  ...
  const context = { ...prepared.context, limits: artifact.limits };
  if (
    context.problem.runtime !== artifact.runtime ||
    context.problem.inputCodec !== artifact.inputCodecVersion ||
    context.problem.outputCodec !== artifact.outputCodecVersion ||
    context.problem.comparisonPolicy !== artifact.comparatorVersion
  ) {
    throw new Error(
      "current problem does not support artifact runtime or versions",
    );
  }
  ...
  return {
    context,
    runner: prepared.runner,
    naive: confined(root, await realpath(`${root}/${artifact.naivePath}`)),
    solution: confined(
      root,
      await realpath(`${root}/${artifact.solutionPath}`),
    ),
  };
}
function predicateOf(
  ...
  const result = createOutputComparator().compare(
    naive.output,
    solution.output,
    {
      version: artifact.comparatorVersion,
      equality: "semantic",
  ...
  return result.equal
    ...
    : {
        ...
        naiveStatus: naive.status,
        solutionStatus: solution.status,
        mismatchPath: result.mismatch.path,
        mismatchReason: result.mismatch.reason,
      };
}
async function persistReplay(
  ...
  await d.transaction.transact(async (uow) => {
    await uow.runs.commit({
      ...
      inputCodecVersion: artifact.inputCodecVersion,
      outputCodecVersion: artifact.outputCodecVersion,
      comparisonPolicyVersion: artifact.comparatorVersion,
      inputHash: sha256Bytes(input),
      outputHash: null,
      generation: initialGeneration(),
      wallTimeUtc: now,
      durationMs: 0,
    ...
    await uow.cases.commit({
      case_id: caseId,
      run_id: String(runId),
      ordinal: artifact.caseIndex,
      input_sha256: sha256Bytes(input),
      input_bytes: BigInt(input.length),
      status: solution.status,
    });
    const problemId = `problem-${artifact.slug}`;
    for (const [role, path, execution] of [
      ["naive", artifact.naivePath, naive],
      ["solution", artifact.solutionPath, solution],
    ] as const) {
      const source = await readFile(`${context.problemRoot}/${path}`, "utf8");
      ...
      await uow.implementations.register({
        ...
        path,
        role,
        content_sha256: sha256Bytes(new TextEncoder().encode(source)),
        runtime: artifact.runtime,
        created_at: now,
      });
      await uow.executions.commit(
        executionRow(identifier(d), caseId, implementationId, execution),
      );
    }
    await uow.replays.commit({
...
function executionRow(
  ...
  return {
    ...
    wall_ns: ns(envelope.wallTimeMs),
    cpu_ns: ns(envelope.cpuTimeMs),
    peak_memory_bytes: BigInt(Math.trunc(envelope.memoryPeakBytes)),
    stdout_bytes: BigInt(envelope.stdoutBytes),
    stderr_bytes: BigInt(envelope.stderrBytes),
...
function ns(value: number): bigint | null {
...
function confined(root: string, path: string): string {
  ...
  if (value === "" || value === ".." || value.startsWith(`..${sep}`))
    throw new Error("recorded asset escapes problem root");
  return value.split(sep).join("/");
}
function samePredicate(a: FailurePredicate, b: FailurePredicate): boolean {
  return (
    a.kind === b.kind &&
    a.naiveStatus === b.naiveStatus &&
    a.solutionStatus === b.solutionStatus &&
    a.mismatchPath === b.mismatchPath &&
    a.mismatchReason === b.mismatchReason
  );
}
function identifier(d: ReplayDependencies): string {
...
function failure(
  ...
  return { status, result: null, diagnostics: [{ code, message }] };
}
