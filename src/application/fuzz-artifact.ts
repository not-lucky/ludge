...
export function serializeFuzzArtifact(
...
export async function writeFuzzArtifact(
  ...
  if (
    storageCapBytes !== undefined &&
    (await directoryBytes(artifactsRoot)) + bytes.length > storageCapBytes
  ) {
...
export async function loadFuzzArtifact(
  path: string,
  expectedId: string,
): Promise<FuzzArtifactDocument> {
  if (
    !/^[a-f0-9]{64}$/u.test(expectedId) ||
    basename(resolve(path, "..")) !== expectedId
  ) {
    throw new FuzzArtifactError("invalid artifact content identifier");
  }
  const bytes = await readFile(path);
  if (sha256(bytes) !== expectedId)
    throw new FuzzArtifactError(
      "artifact SHA-256 does not match its identifier",
    );
  let value: unknown;
  try {
    value = JSON.parse(new TextDecoder().decode(bytes));
  } catch {
    throw new FuzzArtifactError("artifact is not valid JSON");
  }
...
export function assertFuzzArtifactDocument(
  value: unknown,
): asserts value is FuzzArtifactDocument {
  if (
    !isRecord(value) ||
    value.schemaVersion !== FUZZ_ARTIFACT_SCHEMA_VERSION ||
    value.kind !== "fuzz-finding"
  ) {
  ...
  for (const key of [
    ...
    "naivePath",
    "solutionPath",
    "originalInputBase64Url",
    "minimizedInputBase64Url",
    "createdAt",
  ] as const) {
    if (typeof value[key] !== "string" || value[key].length === 0)
      throw new FuzzArtifactError(
        `artifact field ${key} must be a non-empty string`,
      );
  }
  ...
  if (
    ...
    throw new FuzzArtifactError("artifact seed or case index is invalid");
  ...
  if (
    ...
    !isRecord(value.naive) ||
    !isRecord(value.solution) ||
    !isRecord(value.shrink)
  )
    throw new FuzzArtifactError("artifact nested object is invalid");
  assertLimits(value.limits);
  if (
    !["oracle_failure", "optimized_failure", "mismatch"].includes(
      String(value.predicate.kind),
    ) ||
    typeof value.predicate.naiveStatus !== "string" ||
    typeof value.predicate.solutionStatus !== "string" ||
    !(
      typeof value.predicate.mismatchPath === "string" ||
      value.predicate.mismatchPath === null
    ) ||
    !(
      typeof value.predicate.mismatchReason === "string" ||
      value.predicate.mismatchReason === null
    )
  )
    throw new FuzzArtifactError("artifact predicate is invalid");
  const shrink = value.shrink;
  const steps = shrink.steps;
  const originalBytes = shrink.originalBytes;
  const minimizedBytes = shrink.minimizedBytes;
  if (
    typeof shrink.requested !== "boolean" ||
    typeof steps !== "number" ||
    !Number.isSafeInteger(steps) ||
    steps < 0 ||
    !["not_requested", "complete", "step_cap", "time_cap"].includes(
      String(shrink.reason),
    ) ||
    typeof originalBytes !== "number" ||
    !Number.isSafeInteger(originalBytes) ||
    typeof minimizedBytes !== "number" ||
    !Number.isSafeInteger(minimizedBytes) ||
    originalBytes < 0 ||
    minimizedBytes < 0
  )
    throw new FuzzArtifactError("artifact shrink result is invalid");
  for (const envelope of [value.naive, value.solution])
...
function assertLimits(value: Record<string, unknown>): void {
  for (const key of [
    ...
    if (typeof limit !== "number" || !Number.isSafeInteger(limit) || limit <= 0)
      throw new FuzzArtifactError(`artifact limit ${key} is invalid`);
  }
}
...
function assertEnvelope(value: Record<string, unknown>): void {
  for (const key of [
    ...
    if (typeof value[key] !== "string")
      throw new FuzzArtifactError(`artifact envelope ${key} is invalid`);
  for (const key of [
    "stdoutBytes",
    "stderrBytes",
    "wallTimeMs",
  ...
  if (
    ...
    throw new FuzzArtifactError("artifact envelope truncation flag is invalid");
  assertBytes(value.stdoutBase64Url, "stdoutBase64Url");
  assertBytes(value.stderrBase64Url, "stderrBase64Url");
}
function assertBytes(value: unknown, field: string): void {
  if (typeof value !== "string" || !/^[A-Za-z0-9_-]*$/u.test(value))
    throw new FuzzArtifactError(`artifact field ${field} is not base64url`);
}
