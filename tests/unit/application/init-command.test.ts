...
import { loadProblem } from "../../../src/infrastructure/problem.js";
...
describe("init command", () => {
  it("exclusively scaffolds a strict schema-v1 YAML document and registers it", async () => {
    ...
    const yaml = await readFile(
      join(invocationDirectory, "problems", "two-sum", "problem.yaml"),
      "utf8",
    );
    expect(loadProblem(yaml)).toMatchObject({
      slug: "two-sum",
      title: "Two Sum",
      schemaVersion: 1,
      kind: "function",
    ...
    await expect(
      readFile(
        join(invocationDirectory, "problems", "two-sum", "problem.md"),
        "utf8",
      ),
    ).resolves.toContain("# Two Sum");
    await expect(
      readFile(
        join(invocationDirectory, "problems", "two-sum", "solution.py"),
        "utf8",
      ),
    ).resolves.toContain("def solution");
  ...
  it("rejects existing and invalid slugs without modifying the registration", async () => {
    ...
    expect(existing).toMatchObject({
      ...
      diagnostics: [{ code: "problem_exists" }],
    });
    expect(invalid).toMatchObject({
      status: "invalid_input",
      diagnostics: [{ code: "invalid_slug" }],
    });
